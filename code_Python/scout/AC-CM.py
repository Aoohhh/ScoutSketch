        ...
        if flage == False:
            for i in range(self.cell_nums):
                if self.bucket[pos][i][0] == 0:
                    ...
                    #self.bucket[pos][i][2] = (time_windows-1)%(self.T + 1)+3
                    flage = True
                    break
    ...
    def insert_value(self):
        ...
        for idx in range(self.bucket_nums):
            for cell in range(self.cell_nums):
                value = cmsketch.Query(str(self.bucket[idx][cell][0]))
...
if __name__=="__main__":
    ...
    for i in [5,7.5,10,12.5,15]:      # KB
        ...
        with open("E:\\research\\grade1\\B-code\\ScoutSketch\\experiment_results\\on_stack\\straw1\\memory_"+str(i)+"KB.txt", "w") as f:
