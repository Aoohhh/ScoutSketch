    ...
    def insert(self,flow_id):
        ...
        for i in range(self.cell_nums):
            if self.scout[pos][i][0] != 0:
                # if self.scout[pos][i][0] == flow_id:  不可能存在已经存在bucket中的情况
        ...
        #         return
        #     else:
        #         if self.scout[pos][i][1]<temp_max:
        #             temp_max = self.scout[pos][i][1]
        #             temp_cell = i
        # self.scout[pos][temp_cell][0] = flow_id
        # self.scout[pos][temp_cell][5] = time_windows
        ...
        #             temp_max = self.scout[pos][i][1]
        #             temp_cell = i
        # self.scout[pos][temp_cell][1] -= 1
        # if self.scout[pos][temp_cell][1] == 0:
        #     self.scout[pos][temp_cell][0] = flow_id
        #     self.scout[pos][temp_cell][1] = 1
        #     self.scout[pos][temp_cell][5] = time_windows
        ...
        for idx in range(self.bucket_nums):
            for cell in range(self.cell_nums):
                if self.scout[idx][cell][0] != 0: 
                    ...
                    if self.scout[idx][cell][1] > self.scout[idx][cell][2] * self.P:     # 
                        if self.scout[idx][cell][1]<16:   # 16是filter的阈值 # 可能导致bug（起点的值太小，不可以当起点）
                            self.scout[idx][cell][0] = 0
                            self.scout[idx][cell][1] = 0
                            self.scout[idx][cell][2] = 0
                            self.scout[idx][cell][3] = 0
                        ...
                        if self.scout[idx][cell][6] == 0:       #此前为起点或上升
                            ...
                            if self.scout[idx][cell][2] < 16:   # 16是filter的阈值
                        ...
                        if self.scout[idx][cell][6] == 0:   # 此前为起点或上升
                            if self.scout[idx][cell][2] < self.decline_threshold:   # 太小了 不可以继续降 但是需要检查最新值是否大于升的阈值 否则清空
                                if self.scout[idx][cell][1]>=16:  # 把当前点作为一个新的起点
                                    self.scout[idx][cell][2] = self.scout[idx][cell][1]
                                    self.scout[idx][cell][1] = 0
                                    self.scout[idx][cell][3] = 1
                                    self.scout[idx][cell][4] = 0
                        ...
                        for T_value in [10]:
                            ...
                            for q in [0.95]:                            
                                ...
                                for judge in [3]:
                                    ...
                                    with open("E:\\research\\grade1\\result\\(scout-Frequent)memory_"+str(memory)+".txt", "w") as f:
